...
from my_mdp.mdp_bounds import hit_bounds
...
def train_hit_mdp(bounds,load=False,nb_data=10,render=False):
    """
    ...
    mdp = None 
    try:
        mdp = Mdp(HIT_NAME,load)
    except:
        print("NO MDP FILE FOUND")
        mdp = Mdp(HIT_NAME,False)
...
def train_model_hit(render = False,nb_data=10):

    # prends les bornes
    bounds = hit_bounds() 

    # fait une etape d'entrainement
    hit = train_hit_mdp(bounds,load=True,nb_data=nb_data,render=render)

    # sauvegarde
    hit.dump()
    hit.to_explicit_file()
    hit.to_prism_file()


def load_scheduler_mdp_hit(prop):
    hit = Mdp(HIT_NAME)

    model = stormpy.build_sparse_model_from_explicit(transition_file=hit.PATH_TRANSITION, labeling_file=hit.PATH_LABEL)
    print(model)
    ...
    print(scheduler)
    return (result,scheduler,hit_bounds())
