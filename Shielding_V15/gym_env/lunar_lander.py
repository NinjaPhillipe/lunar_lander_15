...
class LunarLander(gym.Env, EzPickle):
    ...
    def __init__(self):
        ...
        # Define fixtures to avoid memory leaks
        self.lander_fixtures = fixtureDef(
        ...
        self.particle_fixtures07 = fixtureDef(
        ...
        self.particle_fixtures35 = fixtureDef(
            shape=circleShape(radius=2/SCALE, pos=(0,0)),
            density=3.5,
            friction=0.1,
            categoryBits=0x0100,
        ...
        self.moon_shape = edgeShape(vertices=[(0, 0), (W, 0)])
        
        self.moon_fixtures = []
        for i in range(CHUNKS - 1):
            self.moon_fixtures.append(
                fixtureDef(
                    shape=edgeShape(vertices=[(chunk_x[i],0),(chunk_x[i+1],0)],
                    )
                )
            )
    ...
    def reset(self):
        ...
        self.moon = self.world.CreateStaticBody( shapes= self.moon_shape )
        self.sky_polys = []
        for i in range(CHUNKS-1):
            p1 = (chunk_x[i],   smooth_y[i])
            p2 = (chunk_x[i+1], smooth_y[i+1])
            self.moon_fixtures[i].shape.vertices = [p1,p2]
            self.moon.CreateFixture(self.moon_fixtures[i],
                                    density=0,
                                    friction=0.1)
    ...
    def _create_particle(self, mass, x, y, ttl):
        if mass == 0.7:
            fixtures = self.particle_fixtures07
        else:
            fixtures = self.particle_fixtures35
